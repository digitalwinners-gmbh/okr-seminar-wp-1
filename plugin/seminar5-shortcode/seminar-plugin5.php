...
Description: A plugin that renders different HTML based on a parameter. v2: layout="okrs" rendert das neue Design aus lokalen Templates; [seminar5_info]/[seminar5_data] liefern Kennzahlen (ab-Preis, Anzahl Termine/Orte).
Version: 2.0
*/

require_once plugin_dir_path(__FILE__) . 'seminar5-okrs.php';
...
function seminar5_shortcode($atts) {
    ...
    $atts = shortcode_atts(array(
        ...
        'layout' => '' // 'okrs' = neues Design (lokale Templates statt Snippet-Seite)
    ), $atts);

    // Neues Layout: komplett eigener Render-Pfad, alter Code bleibt unangetastet
    if ($atts['layout'] === 'okrs') {
        return seminar5_okrs_render($atts['type']);
    }
